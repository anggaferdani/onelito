...
<div class="main-content">
    <section class="section">
        <div class="section-header">
            <h1>Management Notifikasi</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active"><a href="#">Dashboard</a></div>
                <div class="breadcrumb-item">Management Notifikasi</div>
            </div>
        </div>
                    ...
                    <div class="card">
                        <div class="card-body">
                          ...
                          <div class="float-right">
                            <form id="filter" action="{{ route('admin.notifikasi.index') }}" method="GET">
                            ...
                            <table class="table table-bordered">
                              <thead>
                                <tr>
                                  ...
                                  <th class="align-items-center text-center text-nowrap">Deskripsi</th>
                              ...
                              <tbody>
                                @foreach($notifications as $notification)
                                  <tr>
                                    <td class="align-items-center text-center text-nowrap">{{ ($notifications->currentPage() - 1) * $notifications->perPage() + $loop->iteration }}</td>
                                    <td class="align-items-center text-center text-nowrap">{{ $notification->label }}</td>
                                    <td class="align-items-center text-center" style="max-width: 250px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                                      {{ \Illuminate\Support\Str::limit($notification->description, 100, '...') }}
                                    </td>
                                    <td class="align-items-center text-center text-nowrap">
                                      {{ $notification->created_at ? $notification->created_at->timezone('Asia/Jakarta')->translatedFormat('d F Y H:i') : '-' }}
                                    </td>
                                    <td class="align-items-center text-nowrap">
                                      <form action="{{ route('admin.notifikasi.destroy', $notification->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="btn btn-icon btn-primary" data-toggle="modal" data-target="#editModal{{ $notification->id }}"><i class="fas fa-pen"></i></button>
...
<div class="modal fade" id="createModal" data-backdrop="static" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <form action="{{ route('admin.notifikasi.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
...
@foreach($notifications as $notification)
<div class="modal fade" id="editModal{{ $notification->id }}" data-backdrop="static" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
      ...
      <form action="{{ route('admin.notifikasi.update', $notification->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="modal-body">
          <div class="form-group">
            <label for="">Label <span class="text-danger">*</span></label>
            <input type="text" class="form-control" name="label" value="{{ $notification->label }}">
            @error('label')<div class="text-danger">{{ $message }}</div>@enderror
          </div>
          <div class="form-group">
            <label for="">Deskripsi <span class="text-danger">*</span></label>
            <textarea class="form-control" name="description" rows="5" style="height: auto;">{{ $notification->description }}</textarea>
            @error('description')<div class="text-danger">{{ $message }}</div>@enderror
          </div>
          <div class="form-group">
            <label for="">Link</label>
            <input type="text" class="form-control" name="link" value="{{ $notification->link }}">
            @error('link')<div class="text-danger">{{ $message }}</div>@enderror
          </div>
        </div>
        <div class="modal-footer">
