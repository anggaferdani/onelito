...
<div class="main-content">
    <section class="section">
        <div class="section-header">
            <h1>Management Tag</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active"><a href="#">Dashboard</a></div>
                <div class="breadcrumb-item">Management Tag</div>
            </div>
        </div>
                    ...
                    <div class="card">
                        <div class="card-body">
                          ...
                          <div class="float-right">
                            <form id="filter" action="{{ route('admin.tag.index') }}" method="GET">
                          ...
                          <div class="table-responsive">
                            <table class="table table-bordered">
                              ...
                              <tbody>
                                @foreach($tags as $tag)
                                  <tr>
                                    <td class="align-items-center text-center text-nowrap">{{ ($tags->currentPage() - 1) * $tags->perPage() + $loop->iteration }}</td>
                                    <td class="align-items-center text-center text-nowrap">{{ $tag->tag }}</td>
                                    <td class="align-items-center text-nowrap">
                                      <form action="{{ route('admin.tag.destroy', $tag->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="btn btn-icon btn-primary" data-toggle="modal" data-target="#editModal{{ $tag->id }}"><i class="fas fa-pen"></i></button>
...
@foreach($tags as $tag)
<div class="modal fade" id="editModal{{ $tag->id }}" data-backdrop="static" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
