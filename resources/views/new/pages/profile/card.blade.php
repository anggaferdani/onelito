...
@if($auth->member == null)
<div>
  <div class="mb-2">Anda belum memiliki member card. Dengan member card, Anda bisa mendapatkan berbagai keuntungan menarik, termasuk akses prioritas ke promo terbaru, harga spesial, dan informasi eksklusif tentang layanan kami.</div>
  <a href="{{ route('profile.member') }}" class="btn btn-danger">Buat member card</a>
</div>
@else
@php
  $dns2D = new DNS2D();
  $qrCode = $dns2D->getBarcodePNG((string) $auth->member, 'QRCODE');
@endphp
<div class="m-auto rounded d-flex justify-content-between flex-column p-3" style="width: 300px; height: 170px; background-position: center; background-size: cover; background: url({{ asset('img/card.png') }});">
  ...
  <div class="d-flex align-items-center justify-content-between w-100">
    <div class="">
      <div style="font-size: 11px;" class="text-white fw-bold">{{ $auth->member }}</div>
