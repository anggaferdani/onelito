      ...
      <div class="py-3">
        ...
        @forelse($carts as $cart)
          ...
          <div class="d-flex mb-3 gap-2">
            <input class="form-check-input col-1 cart-checkbox" type="checkbox" value="{{ $cart->id_keranjang }}" id="cartCheck_{{ $cart->id_keranjang }}" checked>
            <div class="row g-2">
              ...
              <div class="col-9 col-md-10">
                ...
                <div class="mb-2 fw-bold">Rp. {{ number_format($cartPrice, 0, '.', '.') }}</div>
                <div class="text-danger small mb-2" data-bs-toggle="modal" data-bs-target="#tulisCatatan{{ $cart->id_keranjang }}" style="cursor: pointer;">Tulis Catatan</div>
  ...
  $(document).ready(function() {
    ...
    $selectAll.click(function() {
      if (!$(this).prop('disabled')) {
        $cartCheckboxes.prop('checked', this.checked);
        updateTotalPrice();
      }
    });

    $cartCheckboxes.change(function() {
      const allChecked = $cartCheckboxes.length === $cartCheckboxes.filter(':checked').length;
