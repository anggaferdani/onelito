...
<div class="container-md d-flex flex-column" style="min-height: 100vh;">
    ...
    <div class="flex-grow-1 d-flex flex-column justify-content-center pb-5">
        ...
        <div class="row mb-4">
            <center>
                <h2 class="mb-2">Verifikasi Nomor Telepon</h2>
                <p class="text-muted mb-0" style="width: 100%; max-width: 400px;">
                    Kami telah mengirimkan kode verifikasi ke nomor telepon Anda melalui WhatsApp.
                    Silakan periksa pesan WhatsApp Anda dan masukkan kodenya untuk melanjutkan.
                </p>
            </center>
        </div>
        ...
        <form method="POST" action="{{ route('post-phone-number-verification', ['token' => $member->verification_token]) }}" class="mt-2" autocomplete="off">
            @csrf
            <div class="row">
                <div class="col-md-12">
                    <div class="row">
                        <center>
                            <div style="width: 100%; max-width: 400px;" class="mb-4">
                                <div class="relative">
                                    <input type="text" name="verification_code" id="verification_code" autocomplete="off" required
            ...
            <center>
                <div class="d-grid gap-2 mx-auto mb-2" style="width: 100%; max-width: 400px;">
                ...
                <button type="button" id="request-new-code" class="btn btn-link">Kirim Ulang Kode Verifikasi</button>
            </center>
        </form>
    </div>
</div>
...
<script>
    $(document).ready(function() {
        // Fungsi untuk validasi hanya angka
        $("#verification_code").on('input', function(e) {
            $(this).val($(this).val().replace(/[^0-9]/g, ''));
        });

        $('#request-new-code').click(function(e) {
            e.preventDefault();

            var button = $(this);
            button.prop('disabled', true).text('Mengirim ulang...'); // Disable tombol dan ubah teks

            $.ajax({
                url: "{{ route('request-verification-code', ['token' => $member->verification_token]) }}",
                type: 'POST',  // Penting: Gunakan method POST
                data: {
                   _token: "{{ csrf_token() }}" //Tambahkan CSRF token untuk proteksi
                },
                success: function(response) {
                    alert(response.message); // Tampilkan pesan sukses

                    if (response.redirect) {
                        window.location.href = response.redirect; // Redirect jika kode masih berlaku
                    }

                },
                error: function(xhr, status, error) {
                    alert('Gagal mengirim kode baru. Silakan coba lagi.'); // Tampilkan pesan error
                    console.error(xhr.responseText);
                },
                complete: function() {
                     button.prop('disabled', false).text('Kirim Ulang Kode Verifikasi'); // Enable tombol kembali
                }
            });
        });
    });
